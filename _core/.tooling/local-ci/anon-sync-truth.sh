#!/usr/bin/env bash
# =============================================================================
# ARK 真値 → repo 内 anon-words.txt 同期 (= 真値 1 箇所原則)
# =============================================================================
# ~/haven/ark/rules/anon-words.txt を repo 内 .tooling/local-ci/anon-words.txt
# に cp で複製。 真値は ARK 配下にのみ存在し、 各 repo は読み取り専用 mirror。
#
# 派生 repo / 既存 repo どちらでも実行可。 ARK が存在しない環境 (= deriver の
...
#
# Exit:
#   0 = 同期成功 (= ARK 真値あり) or fallback 成功 (= example で初期化)
...
TRUTH_PATH="${ANON_TRUTH_PATH:-${HOME}/haven/ark/rules/anon-words.txt}"
...
if [ -f "${TRUTH_PATH}" ]; then
    ...
    log_ok "synced anon-words.txt from ARK truth (${TRUTH_PATH})"
    echo "  → ${TARGET_PATH}"
...
log_warn "ARK truth not found at ${TRUTH_PATH}"
...
log_fail "no ARK truth, no example.txt, no existing anon-words.txt — set up manually"
exit 1
